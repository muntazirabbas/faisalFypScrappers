...
def goToProductDetail(_productData,productUrl):
    ...
    #.select('div[class*="swatch-element"]')
    sizeDiv = soup.findAll('div', attrs={'class' : 'swatch clearfix'})[1].select('div[class*="swatch-element"]')
    colorDiv = soup.select('div[class*="swatch-element"]')[0]
    shorColor = colorDiv.find('div', attrs={'class': 'tooltip'}).text.strip()
    color = colorAssignment(shorColor)
    ...
    if(soup.select('#more-view-carousel')[0]):
        pictureDiv = soup.select('#more-view-carousel')[0].findAll('img')
        # print('pic div ', pictureDiv)
